...
; First step: use LButton as base, make a copy of it. uncomment LButton & ~Wheelup, LButton & ~WheelDown, LButton & ~MButton in the copy. Then use the copy as base for all other ahk. BUT do not uncomment that for Lbutton.ahk -- Leave them commented

; * For each modifier.ahk, search for '~LButton & ' and replace it with '~RButton &' '~MButton &' '~XButton1 &' '~XButton2 &'  '!' '^' '+' '#'
; * For middle.ahk, uncomment mbutton & ~rbutton (and del 1 of the 2 ~LButton & MButton, the one right above mbutton & ~rbutton -- NOT the one in custom hotkey section)
...
; * in RButton ahk (already done for LButton), remove as well the custom hotkey ~RButton & ~MButton:: to avoid duplication (since both Magnifier and Custom Hotkey for Left/Right use the Mbutton)
